...
def check_deepseek(query):
    ...
    prompt = (
        ...
        "Ты — ассистент, который проверяет факты и ищет свежие новости.\n"
        "Ты обязан ответить СТРОГО в одном из двух форматов и только на русском языке:\n"
        "1) 'ДА: <краткая суть>' — если факт подтверждён или новость найдена.\n"
        "2) 'НЕТ' — если факт не подтверждён или новостей нет.\n\n"
        "Запрещено писать любые другие слова, рассуждения, переводы или пояснения перед ответом. "
        "Сразу начинай с 'ДА:' или 'НЕТ'.\n"
        "Пример: ДА: Россия открыла прямые рейсы в Японию с 1 сентября."
    )
    payload = {
        "model": "deepseek-chat",
        "input": prompt,
